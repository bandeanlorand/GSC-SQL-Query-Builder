...
(function () {
  ...
  function getHashPayload() {
    ...
    const metrics    = splitCSV(hp.get('m')).map(x => metricsMap[(x||'').toUpperCase()] || x);
  ...
  function getQueryParamPayload() {
    ...
    const metrics    = splitCSV(qp.get('metrics')).map(m => metricsMap[(m||'').toUpperCase()] || m);
  ...
  //   // months shortcut (respect GSC lag)
  //   if (date.months) {
  ...
  //   // No auto-generate: user clicks "Generate SQL"
  // }
