...
(function () {
  ...
  function getHashPayload() {
    ...
    fs.forEach(rawFilter => {
      ...
      let op = opMap[opRaw] || opRaw;
      let v = valueRaw;

      // --- normalize device / country first ---
      if (field === 'Device') {
        v = normalizeDevice(v);
      } else if (field === 'Country') {
        v = normalizeCountryCode(v.replace(/\*/g, ''));
      }

      // --- detect the "*~regex*" / "~regex" pattern used by the extension ---
      // examples:
      //   *~^http(s)?://...$*   ➜ regex "^http(s)?://...$"
      //   ~^foo$                ➜ regex "^foo$"
      const regexMatch = v.match(/^\*?~(.+?)\*?$/);

      if (regexMatch && (op === 'CONTAINS' || op === 'NOT CONTAINS')) {
        // treat as regex filter
        v = regexMatch[1]; // the inner regex only
        op = (op === 'CONTAINS') ? 'REGEXP CONTAINS' : 'NOT REGEXP CONTAINS';
      } else {
        // non-regex: strip "*" wildcards for nicer input on URL / Query
        if (field === 'URL' || field === 'Query') {
          v = v.replace(/\*/g, '');
        }
      }
  ...
  function setFilters(filters = [], logic = 'AND') {
    ...
    filters.forEach((f, idx) => {
      ...
      // const deviceSelect = row.querySelector('select.device-select, select[data-role="filter-device"]');
      // const searchTypeSelect = row.querySelector('select.searchtype-select, select[data-role="filter-searchtype"]');
      ...
      // } else if (searchTypeSelect && (fieldSelect?.value === 'Search Type')) {
      //   searchTypeSelect.classList.remove('hidden');
      //   // IMAGE / DISCOVER / WEB / VIDEO / NEWS...
      //   const code = (f.value || '').toUpperCase();
      //   searchTypeSelect.value = code;
      ...
      // all selects in this row: [field, operator, value]
      const allSelects = row.querySelectorAll('select');
      const valueSelectFallback = allSelects[2] || null;

      const deviceSelect = row.querySelector(
        'select.device-select, select[data-role="filter-device"]'
      ) || valueSelectFallback;

      const searchTypeSelect = row.querySelector(
        'select.searchtype-select, select[data-role="filter-searchtype"]'
      ) || valueSelectFallback;
