...
function buildDropdownItemsHTML(options, currentValue = null) {
  return options.map(o => `
    <div class="cursor-pointer p-1 hover:bg-gray-300 rounded-[3px] px-2 ${o.value === currentValue ? 'bg-gray-200' : ''}"
         data-field="${o.value}">
      ${o.label}
    </div>
  `).join('');
...
/** PUBLIC: toggle Sort By section (keeps your original API) */
export function toggleSortBy() {
...
/** PUBLIC: remove last sort row (keeps your original API) */
export function removeSortRow() {
...
export function addSortRow() {
  ...
  trigger.tabIndex = 0;
  trigger.className = 'input w-full input-lg relative items-center justify-between cursor-pointer focus:outline-none h-auto p-[9px] text-sm';
  trigger.innerHTML = `
    <span id="sortFieldPlaceholder-${idx}" class="text-gray-400">Select Field</span>
  ...
  dropdown.className = 'bg-white border border-gray-600 p-1 rounded absolute z-10 mt-[8px] hidden max-h-[240px] overflow-auto w-full';
...
/** PUBLIC: show/hide the "−" button depending on rows (keeps your API) */
export function updateSortRemoveButton() {
  const container = getSortRowsEl();
  const removeBtn = document.getElementById('removeSortRowBtn');
  if (!removeBtn) return;
